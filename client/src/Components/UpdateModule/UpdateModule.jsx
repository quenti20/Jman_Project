...
const UpdateModule = ({ visibility, moduleData, onHide, onUpdate }) => {
  const [updatedModule, setUpdatedModule] = useState({ ...moduleData });

  const handleUpdate = async () => {
    try {
      const { _id, ...moduleDataWithoutId } = updatedModule;
      await onUpdate({ moduleId: _id, ...moduleDataWithoutId }); // Call onUpdate function with updated module data
      onHide();
